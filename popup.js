      ...
      chrome.tabs.sendMessage(
        ...
        function (response) {
          if (chrome.runtime.lastError) {
            ...
            statusEl.textContent = "Error: Recarga la pagina de Kick.";
            return;
          }
          if (response && response.success) {
            ...
            statusEl.textContent = "Error: No se pudo obtener el tiempo.";
            console.error(
              response ? response.error : "Respuesta vacia del content script."
            );
          }
        }
      );
  ...
  getCurrentTabId((tabId) => {
    getTabData(tabId, (data) => {
      ...
      if (data.endTime <= data.startTime) {
        statusEl.textContent = "Error: El 'Fin' debe ser despues del 'Inicio'.";
        return;
      }
      ...
      // Envío //
      statusEl.textContent = "Enviando orden de descarga...";
...
document.getElementById("clip-last-30").addEventListener("click", () => {
  statusEl.textContent = "Iniciando clip rapido...";
  ...
  getCurrentTabId((tabId) => {
    getTabData(tabId, (data) => {
      ...
      statusEl.textContent = "Enviando orden de clip rapido...";

      chrome.runtime.sendMessage({
        action: "downloadLast30s",
        m3u8Url: data.m3u8Url,
      });
    });
  });
